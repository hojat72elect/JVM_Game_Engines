...
public abstract class QuadtreeNode extends Renderable {
    ...
    public String getQuadtreeCacheKey() {
        ...
        return String.valueOf(chunkConfig.getLod()) + chunkConfig.getLocation().getX() + chunkConfig.getLocation().getY()
                + chunkConfig.getIndex().getX() + chunkConfig.getIndex().getY() + worldPos.toString();
    }

    public String getQuadtreeCacheKey(int lod, Vec2f location, Vec2f index) {

        // LOD|LOC_X|LOC_Y|INDEX_X|INDEX_Y
        return String.valueOf(lod) + location.getX() + location.getY()
                + index.getX() + index.getY() + worldPos.toString();
    }
