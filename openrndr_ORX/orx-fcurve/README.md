...
Note that the bounding box of this last curve will have a width of 2.0 pixels and a height under 1.0
pixel.
...
The `Q` and `C` commands (and their lowercase counterparts) allow us to draw quadratic (one control
point)
...
In this example we used `% 9.0` to loop the time between 0.0 and 9.0, repeating the animation over
and over.
...
Please refer to
its [documentation](https://github.com/openrndr/orx/tree/master/orx-expression-evaluator) for
details on the expression language used.
