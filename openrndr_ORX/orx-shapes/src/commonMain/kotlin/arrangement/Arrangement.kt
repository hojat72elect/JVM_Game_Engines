...
private val ShapeContour.startContourPoint
    get() =
        ContourPoint(
            this,
            0.0,
...
private val ShapeContour.endContourPoint
    get() =
        ContourPoint(
            this,
            1.0,
...
private val YPolarity.opposite
    get() =
        when (this) {
...
data class Arrangement(val shapes: List<ShapeProvider>, val maxIters: Int = 1000) {
        ...
        val candidates = buildList {
            ...
            for (i in shapes.indices) {
                ...
                for (j in i + 1 until shapes.size) {
                    val s2 = shapes[j]
                    val inters = s1.shape.contours.flatMap { c1 ->
                        s2.shape.contours.flatMap { c2 ->
                            c1.intersections(c2)
                        }
                    }
                    for (inter in inters) {
    ...
    private fun createFaces() {
        ...
        while (remainingHalfEdges.isNotEmpty()) {
            ...
            if (iters >= maxIters) {
                error(
                    "Arrangement: A face seems to consist of more than maxIters ($maxIters) edges. This is likely " +
